...
import org.maproulette.services.osm.`object`._
...
/**
  ...
  */
@Singleton
class ChangesetService @Inject()(ws:WSClient, nodeService:NodeService, wayService:WayService, relationService:RelationService, config:Config) {
            ...
            ws.url(url).sign(OAuthCalculator(config.getOSMOauth.consumerKey, accessToken)).post(res) onComplete {
              case Success(uploadResult) =>
                uploadResult.status match {
                  case 200 => p success res
                  case 409 => p failure new ChangeConflictException(s"Conflict found in upload: ${uploadResult.body}. $res")
