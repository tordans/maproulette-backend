...
class DataManager @Inject() (config: Config, db: Database, boundingBoxFinder: BoundingBoxFinder)(
  ...
  def getUserLeaderboard(
    ...
    db.withConnection { implicit c =>
      ...
      SQL"""#${this.leaderboardWithRankSQL(
        userFilter,
        projectList,
        challengeList,
        countryCodeFilter,
    ...
    countryCodeFilter match {
      case Some(ccList) if ccList.nonEmpty =>
        ...
        val boundingBoxes = ccList.map { cc =>
          s"""
                ST_Intersects(t.location, ST_MakeEnvelope(${boundingBoxFinder.boundingBoxforCountry(
            cc
          )}, 4326))
              """
        }
  ...
  def getUserTopChallenges(
    ...
    db.withConnection { implicit c =>
      ...
      val enabledFilter =
        if (onlyEnabled) "AND p.id = sa.project_id AND c.enabled = TRUE and p.enabled = TRUE"
        else ""
      ...
      countryCodeFilter match {
        case Some(ccList) if ccList.nonEmpty =>
          ...
          val boundingBoxes = ccList.map { cc =>
            s"""
                  ST_Intersects(t.location, ST_MakeEnvelope(${boundingBoxFinder
              .boundingBoxforCountry(cc)}, 4326))
                """
          }
          boundingSearch = "t.id = sa.task_id AND (" + boundingBoxes.mkString(" OR ") + ") AND "
        case _ => ""
  ...
  def getLeaderboardForUser(
    ...
    db.withConnection { implicit c =>
      ...
      val sqlSelectWithRank = this.leaderboardWithRankSQL(
        None,
        projectFilter,
        challengeFilter,
        countryCodeFilter,
