...
/**
  * @author mcuthbert
  */
class TaskBundleController @Inject()(override val sessionManager: SessionManager,
                                     override val actionManager: ActionManager,
                                     override val bodyParsers: PlayBodyParsers,
                                     dalManager: DALManager,
                                     components: ControllerComponents)
  extends AbstractController(components) with SessionController with TagsMixin[Task] {
  ...
  def setBundleTaskStatus(bundleId: Long, primaryId: Long, status: Int, comment: String = "", tags: String = ""): Action[AnyContent] = Action.async { implicit request =>
    this.sessionManager.authenticatedRequest { implicit user =>
      ...
      this.dalManager.task.setTaskStatus(tasks, status, user, requestReview, completionResponses, Some(bundleId), Some(primaryId))

      for (task <- tasks) {
        val action = this.dalManager.action.setAction(Some(user), new TaskItem(task.id), TaskStatusSet(status), task.name)
  ...
  def setBundleTaskReviewStatus(id: Long, reviewStatus: Int, comment: String = "", tags: String = ""): Action[AnyContent] = Action.async { implicit request =>
    this.sessionManager.authenticatedRequest { implicit user =>
      ...
      for (task <- tasks) {
        val action = this.dalManager.action.setAction(Some(user), new TaskItem(task.id),
