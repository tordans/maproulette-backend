...
import org.maproulette.session.{SearchChallengeParameters, SearchLocation, SearchParameters, SessionManager, User}
...
class TaskController @Inject()(override val sessionManager: SessionManager,
  ...
  def getTasksInBoundingBox(left: Double, bottom: Double, right: Double, top: Double, limit: Int,
    ...
    this.sessionManager.userAwareRequest { implicit user =>
      SearchParameters.withSearch { p =>
        ...
        val (count, result) = this.dalManager.taskCluster.getTasksInBoundingBox(User.userOrMocked(user), params, limit, offset,
  ...
  def getTasksInCluster(clusterId: Int, numberOfPoints: Int): Action[AnyContent] = Action.async { implicit request =>
    this.sessionManager.userAwareRequest { implicit user =>
      SearchParameters.withSearch { implicit params =>
        Ok(Json.toJson(this.dalManager.taskCluster.getTasksInCluster(clusterId, params, numberOfPoints)))
      }
    }
  }
