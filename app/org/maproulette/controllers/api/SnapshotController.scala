...
/**
  * @author krotstan
  */
class SnapshotController @Inject()(sessionManager: SessionManager,
                               config: Config,
                               actionManager: ActionManager,
                               components: ControllerComponents,
                               snapshotManager: SnapshotManager) extends AbstractController(components) {
  ...
  def recordChallengeSnapshot(challengeId: Long): Action[AnyContent] = Action.async { implicit request =>
    this.sessionManager.userAwareRequest { implicit user =>
      Ok(Json.toJson(snapshotManager.recordChallengeSnapshot(challengeId)))
    }
  }

  def getChallengeSnapshotList(challengeId: Long, includeAllData: Boolean): Action[AnyContent] = Action.async { implicit request =>
    this.sessionManager.userAwareRequest { implicit user =>
      if (includeAllData) {
  ...
  def getChallengeSnapshot(snapshotId: Long): Action[AnyContent] = Action.async { implicit request =>
    this.sessionManager.userAwareRequest { implicit user =>
      ...
      Result(
        ...
        body = HttpEntity.Strict(
          ByteString(
            ...
            "Medium_Total," + Task.statusMap.values.map(v => "Medium_" + v).mkString(",") + "," +
