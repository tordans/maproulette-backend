...
trait TestSpec extends MockitoSugar {
  ...
  val virtualChallenge = VirtualChallenge(1, "VChallenge", DateTime.now(), DateTime.now(), None, 101, SearchParameters(), DateTime.now())
  val surveyDAL = mock[SurveyDAL]
  val projectDAL = mock[ProjectDAL]
  val userDAL = mock[UserDAL]
  val adminUser = User(1, DateTime.now(), DateTime.now(),
  ...
  var writeUser = User(2, DateTime.now(), DateTime.now(),
    OSMProfile(2, "WriteUser", "", "", Location(0, 0), DateTime.now(), RequestToken("", "")),
    List(writeGroup))
  var readUser = User(3, DateTime.now(), DateTime.now(),
    OSMProfile(3, "ReadUser", "", "", Location(0, 0), DateTime.now(), RequestToken("", "")),
    List(readGroup))
  var owner = User(100, DateTime.now(), DateTime.now(),
  ...
  def setupMocks(): Unit = {
    ...
    doAnswer(_ => Some(task1)).when(taskDAL.asInstanceOf[BaseDAL[Long, Task]]).retrieveById(1L, None)
    when(taskDAL.retrieveRootObject(eqM(Right(task1)), any())(any())).thenReturn(Some(project1))
    doAnswer(_ => Some(project1)).when(taskDAL.asInstanceOf[BaseDAL[Long, Task]]).retrieveRootObject(eqM(Right(task1)), any())(any())

    // Mocks for Challenges
    when(challengeDAL.retrieveById(0L, None)).thenReturn(None)
    doAnswer(_ => None).when(challengeDAL.asInstanceOf[BaseDAL[Long, Challenge]]).retrieveById(0L, None)
    when(challengeDAL.retrieveById(1L, None)).thenReturn(Some(challenge1))
    doAnswer(_ => Some(challenge1)).when(challengeDAL.asInstanceOf[BaseDAL[Long, Challenge]]).retrieveById(1L, None)
    ...
    doAnswer(_ => None).when(virtualChallengeDAL.asInstanceOf[BaseDAL[Long, VirtualChallenge]]).retrieveById(0L, None)
    when(virtualChallengeDAL.retrieveById(1, None)).thenReturn(Some(virtualChallenge))
    doAnswer(_ => Some(virtualChallenge)).when(virtualChallengeDAL.asInstanceOf[BaseDAL[Long, VirtualChallenge]]).retrieveById(1L, None)

    // Mocks for Surveys
    when(surveyDAL.retrieveById(0L, None)).thenReturn(None)
    doAnswer(_ => None).when(surveyDAL.asInstanceOf[BaseDAL[Long, Challenge]]).retrieveById(0L, None)
    when(surveyDAL.retrieveById(1L, None)).thenReturn(Some(survey1))
    doAnswer(_ => Some(survey1)).when(surveyDAL.asInstanceOf[BaseDAL[Long, Challenge]]).retrieveById(1L, None)
    ...
    doAnswer(_ => Some(User.superUser)).when(userDAL.asInstanceOf[BaseDAL[Long, User]]).retrieveById(-999L, None)
    when(userDAL.retrieveById(-1L, None)).thenReturn(Some(User.guestUser))
    doAnswer(_ => Some(User.guestUser)).when(userDAL.asInstanceOf[BaseDAL[Long, User]]).retrieveById(-1L, None)
    ...
    doAnswer(_ => Some(adminUser)).when(userDAL.asInstanceOf[BaseDAL[Long, User]]).retrieveById(1L, None)
    when(userDAL.retrieveById(2L, None)).thenReturn(Some(writeUser))
    doAnswer(_ => Some(writeUser)).when(userDAL.asInstanceOf[BaseDAL[Long, User]]).retrieveById(2L, None)
    when(userDAL.retrieveById(3L, None)).thenReturn(Some(readUser))
    doAnswer(_ => Some(readUser)).when(userDAL.asInstanceOf[BaseDAL[Long, User]]).retrieveById(3L, None)
    when(userDAL.retrieveById(100L, None)).thenReturn(Some(owner))
    doAnswer(_ => Some(owner)).when(userDAL.asInstanceOf[BaseDAL[Long, User]]).retrieveById(100L, None)
